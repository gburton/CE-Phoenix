...
?>
  ...
  <div class="row">
    <div class="col">
      <h1 class="display-4 mb-2"><?php echo HEADING_TITLE ?>
<?php if (isset($_GET['zID'])) echo ' <small>' . tep_get_geo_zone_name($_GET['zID']) . '</small>'; ?>
      </h1>
    </div>
...
<?php
    $rows = 0;
    $zones_query_raw = "select a.*, c.countries_name, z.zone_name from zones_to_geo_zones a left join countries c on a.zone_country_id = c.countries_id left join zones z on a.zone_id = z.zone_id where a.geo_zone_id = " . $_GET['zID'] . " order by association_id";
...
?>
      <div class="row">
        <div class="col"><p class="pt-2 text-right"><?php echo tep_draw_bootstrap_button(IMAGE_BACK, 'fas fa-angle-left', tep_href_link('geo_zones.php', 'zpage=' . $_GET['zpage'] . '&zID=' . $_GET['zID']), null, null, 'btn-light mr-2') . tep_draw_bootstrap_button(IMAGE_INSERT, 'fas fa-plus', tep_href_link('geo_zones.php', 'zpage=' . $_GET['zpage'] . '&zID=' . $_GET['zID'] . '&action=list&spage=' . $_GET['spage'] . '&' . (isset($sInfo) ? 'sID=' . $sInfo->association_id . '&' : '') . 'saction=new'), null, null, 'btn-warning'); ?></p></div>
      </div>
<?php
    } 
?>
  ...
  if ($action == 'list') {
    switch ($saction) {
      ...
      case 'delete':
        ...
        $contents[] = ['class' => 'text-center', 'text' => '<br>' . tep_draw_bootstrap_button(IMAGE_DELETE, 'fas fa-trash', null, 'primary', null, 'btn-danger xxx text-white mr-2') . tep_draw_bootstrap_button(IMAGE_CANCEL, 'fas fa-times', tep_href_link('geo_zones.php', 'zpage=' . $_GET['zpage'] . '&zID=' . $_GET['zID'] . '&action=list&spage=' . $_GET['spage'] . '&sID=' . $sInfo->association_id), null, null, 'btn-light')];
        break;
      default:
        if (isset($sInfo) && is_object($sInfo)) {
          $heading[] = ['text' => '<strong>' . $sInfo->countries_name . '</strong>'];

          $contents[] = ['align' => 'center', 'text' => tep_draw_bootstrap_button(IMAGE_EDIT, 'fas fa-cogs', tep_href_link('geo_zones.php', 'zpage=' . $_GET['zpage'] . '&zID=' . $_GET['zID'] . '&action=list&spage=' . $_GET['spage'] . '&sID=' . $sInfo->association_id . '&saction=edit'), null, null, 'btn-warning mr-2') . tep_draw_bootstrap_button(IMAGE_DELETE, 'fas fa-trash', tep_href_link('geo_zones.php', 'zpage=' . $_GET['zpage'] . '&zID=' . $_GET['zID'] . '&action=list&spage=' . $_GET['spage'] . '&sID=' . $sInfo->association_id . '&saction=delete'), null, null, 'btn-danger xxx text-white')];
          $contents[] = ['text' => TEXT_INFO_DATE_ADDED . ' ' . tep_date_short($sInfo->date_added)];
          if (tep_not_null($sInfo->last_modified)) $contents[] = ['text' => TEXT_INFO_LAST_MODIFIED . ' ' . tep_date_short($sInfo->last_modified)];
        }
        break;
    }
  } else {
    switch ($action) {
      case 'new_zone':
        ...
        $contents[] = ['align' => 'center', 'text' => '<br>' . tep_draw_bootstrap_button(IMAGE_SAVE, 'fas fa-save', null, 'primary', null, 'btn-success xxx text-white mr-2') . tep_draw_bootstrap_button(IMAGE_CANCEL, 'fas fa-times', tep_href_link('geo_zones.php', 'zpage=' . $_GET['zpage'] . (isset($_GET['zID']) ? '&zID=' . $_GET['zID'] : '')), null, null, 'btn-light')];
        break;
      case 'edit_zone':
        ...
        $contents[] = ['align' => 'center', 'text' => '<br>' . tep_draw_bootstrap_button(IMAGE_SAVE, 'fas fa-save', null, 'primary', null, 'btn-success xxx text-white mr-2') . tep_draw_bootstrap_button(IMAGE_CANCEL, 'fas fa-times', tep_href_link('geo_zones.php', 'zpage=' . $_GET['zpage'] . '&zID=' . $zInfo->geo_zone_id), null, null, 'btn-light')];
        break;
      case 'delete_zone':
        ...
        $contents[] = ['class' => 'text-center', 'text' => '<br>' . tep_draw_bootstrap_button(IMAGE_DELETE, 'fas fa-trash', null, 'primary', null, 'btn-danger xxx text-white mr-2') . tep_draw_bootstrap_button(IMAGE_CANCEL, 'fas fa-times', tep_href_link('geo_zones.php', 'zpage=' . $_GET['zpage'] . '&zID=' . $zInfo->geo_zone_id), null, null, 'btn-light')];
        break;
      default:
        if (isset($zInfo) && is_object($zInfo)) {
          ...
          $contents[] = ['text' => TEXT_INFO_NUMBER_ZONES . ' ' . $zInfo->num_zones];
          $contents[] = ['text' => TEXT_INFO_DATE_ADDED . ' ' . tep_date_short($zInfo->date_added)];
          if (tep_not_null($zInfo->last_modified)) $contents[] = ['text' => TEXT_INFO_LAST_MODIFIED . ' ' . tep_date_short($zInfo->last_modified)];
          $contents[] = ['text' => TEXT_INFO_ZONE_DESCRIPTION . '<br>' . $zInfo->geo_zone_description];
        }
        break;
    }
  }
