  ...
  class ht_grid_list_view {
    ...
    function execute() {
      ...
      if (tep_not_null(MODULE_HEADER_TAGS_GRID_LIST_VIEW_PAGES)) {
        ...
        if (in_array(basename($PHP_SELF), $pages_array)) {
          ...
          $oscTemplate->addBlock('<script>$(function(){var cc = $.cookie(\'list_grid\');if (cc == \'grid\'){$("#grid").find("span").toggleClass("glyphicon glyphicon-th-list");$("#product-listing .inline-span").toggleClass(\''.MODULE_HEADER_TAGS_GRID_LIST_VIEW_DEFAULT.'-across\').toggleClass(\'fluid-'.MODULE_HEADER_TAGS_GRID_LIST_VIEW_MOBILE.'-across\');}else{$("#grid").find("span").toggleClass("glyphicon glyphicon-th");$("#product-listing .inline-span").toggleClass("one-across").toggleClass("fluid-one-across");$.cookie(\'list_grid\', \'list\');};$("#grid").click(function(event){event.preventDefault();$("#grid").find("span").toggleClass("glyphicon-th-list glyphicon-th");$("#product-listing .inline-span").toggleClass(\'one-across '.MODULE_HEADER_TAGS_GRID_LIST_VIEW_DEFAULT.'-across\').toggleClass(\'fluid-one-across fluid-'.MODULE_HEADER_TAGS_GRID_LIST_VIEW_MOBILE.'-across\');if(cc == \'list\'){$.cookie(\'list_grid\', \'grid\');}if(cc == \'grid\'){$.cookie(\'list_grid\', \'list\');}});});</script>' . "\n", $this->group);
        }
      }
    }
    ...
    function install() {
      tep_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, set_function, date_added) values ('Enable Grid List javascript', 'MODULE_HEADER_TAGS_GRID_LIST_VIEW_STATUS', 'True', 'Do you want to enable the Grid/List Javascript module?', '6', '1', 'tep_cfg_select_option(array(\'True\', \'False\'), ', now())");
	    ...
	    tep_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, date_added) values ('Sort Order', 'MODULE_HEADER_TAGS_GRID_LIST_VIEW_SORT_ORDER', '900', 'Sort order of display. Lowest is displayed first.', '6', '5', now())");
    }
