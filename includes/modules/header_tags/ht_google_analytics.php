  ...
  _gaq.push([\'_trackPageview\']);' . "\n";
    ...
    \'' . tep_output_string_protected($order['billing_country']) . '\' // country
            ...
            while ($order_products = tep_db_fetch_array($order_products_query)) {
              $category_query = tep_db_query("select cd.categories_name from " . 'categories_description' . " cd, " . TABLE_PRODUCTS_TO_CATEGORIES . " p2c, " . 'languages' . " l where p2c.products_id = '" . (int)$order_products['products_id'] . "' and p2c.categories_id = cd.categories_id and l.code = '" . tep_db_input(DEFAULT_LANGUAGE) . "' and l.languages_id = cd.language_id limit 1");
