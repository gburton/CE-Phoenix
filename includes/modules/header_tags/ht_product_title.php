      ...
      if ( (basename($PHP_SELF) == FILENAME_PRODUCT_INFO) || (basename($PHP_SELF) == 'product_reviews.php') ) {
        if (isset($HTTP_GET_VARS['products_id'])) {
          if ($product_check['total'] > 0) {
            $product_info_query = tep_db_query("select pd.products_name from " . products . " p, " . products_description . " pd where p.products_status = '1' and p.products_id = '" . (int)$HTTP_GET_VARS['products_id'] . "' and pd.products_id = p.products_id and pd.language_id = '" . (int)$languages_id . "'");
