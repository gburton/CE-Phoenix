    ...
    function execute() {
      ...
      if (tep_not_null(MODULE_HEADER_TAGS_DATEPICKER_JQUERY_PAGES)) {
        ...
        if (in_array(basename($PHP_SELF), $pages_array)) {
          ...
          $oscTemplate->addBlock('<script src="ext/datepicker/locales/bootstrap-datepicker.' . JQUERY_DATEPICKER_I18N_CODE . '.min.js"></script>' . "\n", $this->group);
          $oscTemplate->addBlock('<link rel="stylesheet" href="ext/datepicker/css/bootstrap-datepicker.min.css" />' . "\n", 'header_tags');
          // create_account
          // account edit
          $oscTemplate->addBlock('<script>$(\'#dob\').datepicker({language: \'' . JQUERY_DATEPICKER_I18N_CODE . '\', format: \'' . BOOTSTRAP_DATEPICKER_FORMAT . '\', startView: 2, autoclose: true});</script>', $this->group);
          // advanced search
          $oscTemplate->addBlock('<script>var nowTemp = new Date(); var now = new Date(nowTemp.getFullYear(), nowTemp.getMonth(), nowTemp.getDate(), 0, 0, 0, 0); $(\'#dfrom\').datepicker({language: \'' . JQUERY_DATEPICKER_I18N_CODE . '\', format: \'' . BOOTSTRAP_DATEPICKER_FORMAT . '\', autoclose: true, endDate: now}); </script>', $this->group);
