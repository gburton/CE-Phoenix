    ...
    function execute() {
      ...
      if (tep_not_null(MODULE_HEADER_TAGS_DATEPICKER_JQUERY_PAGES)) {
        ...
        if (in_array(basename($PHP_SELF), $pages_array)) {
          $oscTemplate->addBlock('<script src="ext/datepicker/js/bootstrap-datepicker.min.js"></script>' . PHP_EOL, $this->group);
          $oscTemplate->addBlock('<script src="ext/datepicker/locales/bootstrap-datepicker.' . JQUERY_DATEPICKER_I18N_CODE . '.min.js"></script>' . PHP_EOL, $this->group);
          $oscTemplate->addBlock('<link rel="stylesheet" href="ext/datepicker/css/bootstrap-datepicker.min.css" />' . PHP_EOL, 'header_tags');
          ...
          // create_account
          // account edit
          $oscTemplate->addBlock('<script>$(\'#dob\').datepicker({language: \'' . JQUERY_DATEPICKER_I18N_CODE . '\', format: \'' . BOOTSTRAP_DATEPICKER_FORMAT . '\', startView: 2, autoclose: true,  endDate: now});</script>', $this->group);
          // advanced search
          $oscTemplate->addBlock('<script>$(\'#dfrom\').datepicker({language: \'' . JQUERY_DATEPICKER_I18N_CODE . '\', format: \'' . BOOTSTRAP_DATEPICKER_FORMAT . '\', autoclose: true, endDate: now}); </script>', $this->group);
          $oscTemplate->addBlock('<script>$(\'#dto\').datepicker({language: \'' . JQUERY_DATEPICKER_I18N_CODE . '\', format: \'' . BOOTSTRAP_DATEPICKER_FORMAT . '\', autoclose: true,  endDate: now});</script>', $this->group);
        }
      }
    }
