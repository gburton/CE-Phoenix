    ...
    function execute() {
      ...
      if (basename($PHP_SELF) == 'index.php') {
        if (isset($HTTP_GET_VARS['manufacturers_id']) && is_numeric($HTTP_GET_VARS['manufacturers_id'])) {
          $manufacturers_query = tep_db_query("select manufacturers_name, manufacturers_seo_title from manufacturers where manufacturers_id = '" . (int)$HTTP_GET_VARS['manufacturers_id'] . "'");
          if (tep_db_num_rows($manufacturers_query)) {
            $manufacturers = tep_db_fetch_array($manufacturers_query);

            if (tep_not_null($manufacturers['manufacturers_seo_title'])) {
              $oscTemplate->setTitle($manufacturers['manufacturers_seo_title'] . MODULE_HEADER_TAGS_MANUFACTURER_TITLE_SEPARATOR . $oscTemplate->getTitle());
            }
            else {
