    ...
    function execute() {
      ...
      if ($random_product = tep_random_select("select products_id, products_image, products_tax_class_id, products_price from products where products_status = '1' order by products_date_added desc limit " . MAX_RANDOM_SELECT_NEW)) {
        ...
        if (tep_not_null($random_product['specials_new_products_price'])) {
          ...
          $whats_new_price .= '<span class="productPrice text-danger productSpecialPrice">' . $currencies->display_price($random_product['specials_new_products_price'], tep_get_tax_rate($random_product['products_tax_class_id'])) . '</span>';
        } else {
