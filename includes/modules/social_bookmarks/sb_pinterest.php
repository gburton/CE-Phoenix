...
// and image (used for media)
      $image_query = tep_db_query("select products_image from " . products . " where products_id = '" . (int)$HTTP_GET_VARS['products_id'] . "'");
      $image = tep_db_fetch_array($image_query);

      if (tep_not_null($image['products_image'])) {
        $image_file = $image['products_image'];

        $pi_query = tep_db_query("select image from " . products_images . " where products_id = '" . (int)$HTTP_GET_VARS['products_id'] . "' order by sort_order");
...
// url
      $params['url'] = tep_href_link('product_info.php', 'products_id=' . $HTTP_GET_VARS['products_id'], 'NONSSL', false);
