...
*/
  ...
  class cm_in_new_products {
    ...
    function execute() {
		...
		$new_products_query = tep_db_query("select distinct p2c.products_id from products p, products_to_categories p2c, categories c where c.parent_id = '" . $OSCOM_Category->getID() . "' and c.categories_id = p2c.categories_id and p2c.products_id = p.products_id and p.products_status = '1' order by p.products_date_added desc limit " . (int)MODULE_CONTENT_IN_NEW_PRODUCTS_MAX_DISPLAY);
