...
<?php
	...
	foreach ( $data as $product ) {				
		...
		$content .= '					<a href="' . tep_href_link($PHP_SELF, tep_get_all_get_params(array('action')) . 'action=buy_now&products_id=' . $product['id']) . '" data-has-attributes="' . (( $product['attributes'] > 0 ) ? 1:0) . '" data-in-stock="' . $product['display_quanity'] . '" data-product-id="' . $product['id'] . '" class="btn btn-success btn-index btn-buy" role="button">' . MODULE_CONTENT_NEW_PRODUCTS_BUTTON_BUY . '</a>';
		$content .= '				</div>';
