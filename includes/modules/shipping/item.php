        ...
        for ($i=0, $n=sizeof($order->products); $i<$n; $i++) {
          ...
          if (isset($order->products[$i]['attributes'])) {
            ...
            while (list($option, $value) = each($order->products[$i]['attributes'])) {
              $virtual_check_query = tep_db_query("select count(*) as total from " . TABLE_PRODUCTS_ATTRIBUTES . " pa, " . 'products_attributes_download' . " pad where pa.products_id = '" . (int)$order->products[$i]['id'] . "' and pa.options_values_id = '" . (int)$value['value_id'] . "' and pa.products_attributes_id = pad.products_attributes_id");
