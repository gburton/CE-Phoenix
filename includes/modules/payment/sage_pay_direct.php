      ...
      if ( isset($_GET['check']) ) {
        ...
        if ( !isset($sagepay_token) ) {
          ...
          if ( $cc_type != 'PAYPAL' ) {
            ...
            $cc_number = isset($_POST['cc_number_nh-dns']) ? substr(preg_replace('/[^0-9]/', '', $HTTP_POST_VARS['cc_number_nh-dns']), 0, 20) : null;
            $cc_start = null;
            $cc_expires = null;
            ...
            if ( (($cc_type == 'MAESTRO') && (MODULE_PAYMENT_SAGE_PAY_DIRECT_ALLOW_MAESTRO == 'True')) || (($cc_type == 'AMEX') && (MODULE_PAYMENT_SAGE_PAY_DIRECT_ALLOW_AMEX == 'True')) ) {
              ...
              $cc_start = substr($_POST['cc_starts_month'] . $HTTP_POST_VARS['cc_starts_year'], 0, 4);
            }
            ...
            $cc_expires = substr($_POST['cc_expires_month'] . $HTTP_POST_VARS['cc_expires_year'], 0, 4);
      ...
      if (tep_not_null(MODULE_PAYMENT_SAGE_PAY_DIRECT_DEBUG_EMAIL)) {
        ...
        if (!empty($_POST)) {
          if (isset($_POST['cc_number_nh-dns'])) {
            $HTTP_POST_VARS['cc_number_nh-dns'] = 'XXXX' . substr($_POST['cc_number_nh-dns'], -4);
          }
