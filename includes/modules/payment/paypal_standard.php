...
// remove headers from the encrypted result
    ...
    function before_process() {
      global $oscTemplate, $customer_id, $order, $order_totals, $sendto, $billto, $languages_id, $payment, $currencies, $cart, $cart_PayPal_Standard_ID, $$payment, $HTTP_GET_VARS, $HTTP_POST_VARS, $messageStack;
...
// lets start with the email confirmation
      $oscTemplate->getContent('email_paypal_standard');
...
EOD;
    ...
    function verifyTransaction($is_ipn = false) {
      ...
      if ( isset($HTTP_POST_VARS['invoice']) && is_numeric($HTTP_POST_VARS['invoice']) && ($HTTP_POST_VARS['invoice'] > 0) && isset($HTTP_POST_VARS['custom']) && is_numeric($HTTP_POST_VARS['custom']) && ($HTTP_POST_VARS['custom'] > 0) ) {
        $order_query = tep_db_query("select orders_id, orders_status, currency, currency_value from " . orders . " where orders_id = '" . (int)$HTTP_POST_VARS['invoice'] . "' and customers_id = '" . (int)$HTTP_POST_VARS['custom'] . "'");
