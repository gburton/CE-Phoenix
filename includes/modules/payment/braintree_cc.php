  ...
  class braintree_cc {
    ...
    function before_process() {
          ...
          if ( tep_db_num_rows($token_query) == 1 ) {
            ...
            if ( MODULE_PAYMENT_BRAINTREE_CC_VERIFY_WITH_CVV == 'True' ) {
              ...
              if ( isset($_POST['token_cvv']) && is_array($_POST['token_cvv']) && isset($_POST['token_cvv'][$_POST['braintree_card']]) ) {
                $braintree_token_cvv = $HTTP_POST_VARS['token_cvv'][$_POST['braintree_card']];
              }
      ...
      if ( !isset($braintree_token) ) {
        $cc_owner = isset($_POST['name']) ? $HTTP_POST_VARS['name'] : null;
        $cc_number = isset($_POST['number']) ? $HTTP_POST_VARS['number'] : null;
        $cc_expires_month = isset($_POST['month']) ? $HTTP_POST_VARS['month'] : null;
        $cc_expires_year = isset($_POST['year']) ? $HTTP_POST_VARS['year'] : null;

        if ( MODULE_PAYMENT_BRAINTREE_CC_VERIFY_WITH_CVV == 'True' ) {
          $cc_cvv = isset($_POST['cvv']) ? $HTTP_POST_VARS['cvv'] : null;
        }
