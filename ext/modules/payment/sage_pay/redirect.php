...
// if the customer is not logged on, redirect them to the login page
  ...
  if ( isset($_GET['payment_error']) && tep_not_null($_GET['payment_error']) ) {
    $redirect_url = tep_href_link('checkout_payment.php', 'payment_error=' . $HTTP_GET_VARS['payment_error'] . (isset($_GET['error']) && tep_not_null($_GET['error']) ? '&error=' . $HTTP_GET_VARS['error'] : ''), 'SSL');
  } else {
    $hidden_params = '';
