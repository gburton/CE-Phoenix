...
if ($ok)
{
    $my_order_query = tep_db_query("select orders_status, currency, currency_value from ".orders." where orders_id = '".$HTTP_POST_VARS['order_id']."'"); // TODO: fix PB to add all params"' and customers_id = '" . (int)$HTTP_POST_VARS['custom'] . "'");
...
if ($ok)
{
    ...
    $order = $my_order;
    $total_query = tep_db_query("select value from ".orders_total." where orders_id = '".$HTTP_POST_VARS['order_id']."' and class = 'ot_total' limit 1");
...
if ($result == 'VERIFIED')
{
    ...
    tep_db_query("update ".orders." set orders_status = '".$order_status_id."', last_modified = now() where orders_id = '".(int)$HTTP_POST_VARS['order_id']."'");
    if ($invoice_approved)
    {
    	// for email
        ...
        $comment_query = tep_db_query("select comments from ".orders_status_history." where orders_id = '".$HTTP_POST_VARS['order_id']."'");
        ...
        if (DOWNLOAD_ENABLED == 'true')
        {
            $content_query = tep_db_query("select * from ".orders_products_download." where orders_id = '".(int)$HTTP_POST_VARS['order_id']."'");
        ...
        EMAIL_TEXT_INVOICE_URL.' '.tep_href_link('account_history_info.php', 'order_id='.$HTTP_POST_VARS['order_id'], 'SSL', false)."\n".
...
} else
{
    ...
    if ( isset ($HTTP_POST_VARS['order_id']) && is_numeric($HTTP_POST_VARS['order_id']) && ($HTTP_POST_VARS['order_id'] > 0))
    {
        $check_query = tep_db_query("select orders_id from ".orders." where orders_id = '".$HTTP_POST_VARS['order_id']."'"); //TODO: fix custom "' and customers_id = '" . (int)$HTTP_POST_VARS['custom'] . "'");
        ...
        if (tep_db_num_rows($check_query) > 0)
        {
            ...
            //tep_db_query("update ".orders." set orders_status = '".((MODULE_PAYMENT_INPAY_ORDER_STATUS_ID > 0)?MODULE_PAYMENT_INPAY_ORDER_STATUS_ID:DEFAULT_ORDERS_STATUS_ID)."', last_modified = now() where orders_id = '".$HTTP_POST_VARS['order_id']."'");
            $sql_data_array = array ('orders_id'=>$HTTP_POST_VARS['order_id'],
            'orders_status_id'=>$order_status_id,
            'date_added'=>'now()',
            'customer_notified'=>'0',
