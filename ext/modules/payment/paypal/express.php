  ...
  switch ($_GET['osC_Action']) {
    ...
    case 'callbackSet':
      if (MODULE_PAYMENT_PAYPAL_EXPRESS_INSTANT_UPDATE == 'True') {
        ...
        if (isset($_POST['CURRENCYCODE']) && $currencies->is_set($_POST['CURRENCYCODE']) && ($currency != $HTTP_POST_VARS['CURRENCYCODE'])) {
          $currency = $HTTP_POST_VARS['CURRENCYCODE'];
        }

        while (true) {
          if (isset($_POST['L_NUMBER' . $counter])) {
            $cart->add_cart($_POST['L_NUMBER' . $counter], $HTTP_POST_VARS['L_QTY' . $counter]);
          } else {
            break;
          }
